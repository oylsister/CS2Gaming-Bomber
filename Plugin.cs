...
namespace Bomber
{
    ...
    {
        public override string ModuleName => "The Bomber Acheivement";
        ...
        public Dictionary<CCSPlayerController, PlayerGreandeKillCount> _playerKillCount { get; set; } = new();
        ...
        public void CountKill(CCSPlayerController client)
        {
            ...
            if (Config.MaxGrenadeKillCount <= 0)
                return;
            ...
            if (_playerKillCount[client!].KillCount >= Config.MaxGrenadeKillCount)
            {
                var steamid = client.AuthorizedSteamID?.SteamId64;
        ...
        public async Task TaskComplete(CCSPlayerController client, ulong steamid)
        {
            ...
            if (response != null)
            {
                ...
                Server.NextFrame(() =>
                {
                    client.PrintToChat($" {ChatColors.Green}[Acheivement]{ChatColors.Default} You acheive 'The Bomber' (Kill enemies with greande for {Config.MaxGrenadeKillCount} times)");
